...
(async function () {
  ...
  async function main() {
    const accountToInpersonate = "0xe0ac4559739bd36f0913fb0a3f5bfc19bcbacd52"
    ...
    let proposalNumber = proposeEvents[12]["args"]["proposalId"];
    console.log(proposalNumber);
    console.log(proposalNumber.toString())
    ...
    await governanceContract.castVote(proposalNumber.toString(),"1")
    ...
    for (let i = 0; i<largeVoters.length;i++){
        await hre.network.provider.request({
          ...
          await governanceContract.connect(signer).castVote(8,1)
    }

    await advanceBlockHeight(13000); // fast forward through voting period

    await governanceContract.queue(Number(proposalNumber));
    
    proposalInfo = await governanceContract.proposals(Number(proposalNumber));
    ...
    await governanceContract.execute(Number(proposalNumber));
